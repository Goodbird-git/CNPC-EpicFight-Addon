...
public class AdvNpcPatchReloader  extends SimpleJsonResourceReloadListener {
    ...
    public static AdvNpcPatchProvider deserializeMobPatchProvider(CompoundTag tag, boolean clientSide) {
        ...
        if (EpicFightMod.isPhysicalClient()) {
            ...
            armature = Armatures.getOrCreateArmature(mc.getResourceManager(), armatureLocation, HumanoidArmature::new);
        }else {
            armature = Armatures.getOrCreateArmature((ResourceManager)null, armatureLocation, HumanoidArmature::new);
        }
